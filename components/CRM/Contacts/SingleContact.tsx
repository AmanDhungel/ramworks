...
import ProjectDashboard from "./ContactTabs";
  ...
  return (
    <>
      <div className="flex justify-between mt-2">
        <p className="text-sm text-gray-500 flex items-center gap-1 2">
          <ArrowLeft onClick={() => router.back()} /> Contacts / CRM /{" "}
          <span className="text-gray-900">{data?.data.name}</span>
        </p>
        <div className="flex gap-4">
          <AddDealDialog />
          <Button variant="outline" className="h-9 gap-2 text-white bg-black">
            <MailIcon /> Send Email
          </Button>
        </div>
      </div>
